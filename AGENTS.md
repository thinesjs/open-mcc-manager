...
| A command carrying a credential refused before anything stores it, while the console still sends it | `apps/server/src/instance.test.ts` — drives **Set scheduled command** over HTTP against a real database: `/login hunter2` is refused 400 `INSTANCE_COMMAND_STORES_CREDENTIAL`, the whole wire answer compared, and the caller is then demoted to **`viewer`** and `listScheduledCommands` read back empty — that read is the surface the defect had, because `toScheduledCommandPublic` returns `row.command` verbatim and the procedure requires only `instance.read`. A second case sends the **same** string through `instance.sendCommand` and requires a **200** there beside the schedule's refusal, so moving the check into `sendableLine`, which both doors read, fails. `packages/core/src/instance/control.test.ts` pins the set `refuseStoredCredential` refuses — ten forms including a bare `login`, `//login`, `/l`, `/register`, `/unregister`, `/authme changepassword` and `/email setpassword` — against six it stores, among them `/login` with **no argument** and `/logistics open`, so a refusal that refuses everything fails rather than passing on the refusals alone; a second case names four it refuses that carry no password (`/cp`, `/log`, `/l`, `/reg` with any argument at all), and a third requires `sendableLine("/login hunter2")` to still answer `//login hunter2`. `packages/core/src/instance/instance.controller.test.ts` requires the save to reject with **nothing upserted and nothing audited**. `apps/web/src/lib/errors.test.ts` compares the sentence whole, requires it **not** to match `/reach|server/i`, and requires it to differ from `INSTANCE_COMMAND_DOUBLE_SLASH`'s, so the two refusals cannot be given one reason. What it does NOT hold: **a row an earlier build already stored**. Nothing sweeps `instanceCommand`, whose `command` column is plain `text` under a not-empty and a single-line CHECK and nothing else, so a credential saved before this change still lists verbatim to a viewer until someone deletes it; only the deletion path is driven, and it audits that row masked. Nor the live door's policy beyond that one 200 — an operator may still type any credential at the console, deliberately. Nor any credential syntax outside `COMMAND_PATTERNS`: the refusal **is** `maskCommandCredentials(command) !== command`, so that list is the whole policy and nothing measures whether it is the right one |
...
- **What may be sent is not what may be stored.** `sendableLine` answers whether a
  ...
  alone. Refusing at save rather than masking on read is deliberate: a masked
  schedule is one an operator cannot edit while the scheduler still runs the real
  command, so nothing sensitive being written is the only version of this that
  holds. It does **not** remove a row an earlier build stored — nothing sweeps
  `instanceCommand`, and deleting an operator's schedule out from under them is a
  decision nobody has made. The policy is exactly
  `maskCommandCredentials(command) !== command`, so `COMMAND_PATTERNS` is all of
  it, and it is broader than "a password": any argument to `login`, `l`, `log`,
  `reg`, `register`, `unreg`, `unregister`, `cp`, `changepass`, `changepassword`,
  `authme register|reg|r|password|changepass|changepassword|cp` or
  `email setpassword`, behind up to two leading `/` or `!`, is now unsavable.
