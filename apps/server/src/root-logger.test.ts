...
describe("the root logger this daemon actually constructs at startup", () => {
	it("names itself open-mcc-server on every line, so the two daemons cannot be confused", async () => {
		const { rootLogger, SERVICE } = await loadRoot("info")

		rootLogger.info("a line")

		expect(SERVICE).toBe("open-mcc-server")
		expect(JSON.parse(lines[0] ?? "{}").service).toBe("open-mcc-server")
	})
