...
### A health read the host refused is not a healthy host

`observeHost` asks the host's own user manager what has failed. That question
used to be asked in a way the host could not answer badly: `systemctl --user
list-units … 2>/dev/null | grep -c . || printf 0`. Verified against systemd 252
on Debian 12, with the account's bus taken away: the whole command still exits
**0** and prints `0`, because `grep -c` on empty input exits 1 and `printf 0`
then succeeds. So a host whose user manager had gone was recorded with **zero**
failed units, byte-identical to a host where nothing had failed — the same shape
condemned in the journal section, on the readout the dashboard's badge is built
from. Reading the exit status alone would not have found it; the command had to
stop swallowing it.

The read is now the bare `systemctl --user list-units`, with no redirect, no
pipe and no `|| printf 0`, and the count is one per line it listed. Verified on
the same host: nothing failed prints nothing and exits 0, one failed unit prints
one line and exits 0, no user bus exits 1 with `Failed to connect to bus` on
stderr. A non-zero exit is recorded as `failedUnits: null` — the column and the
contract have always been nullable, and `null` is the only value that means *not
known*.

The dashboard therefore has a fifth health state. A refused read is **not**
unreachable: SSH connected and a command ran, so calling the host offline would
be a second lie, and would raise a `host.unreachable` alert whose sentence — "A
machine stops responding" — is false. It is not `unknown` either, which means
never reached. It reads `unreadable`, badged "Not readable", beside a `lastSeenAt`
this manager may honestly stamp. `observeHost` no longer returns `reachable` at
all: it is handed an open reader, so it cannot observe reachability, and the
field only ever restated what `runHealthPoll` already knew from whether the lease
or the exec threw.
...
`-n` bounds **lines**, and journalctl offers nothing that bounds bytes — read its
`--help` and the only levers on output size are `-n`, `-o` and `--no-full`. A
line is bounded only by journald's own `LineMax`, 48 KB by default, so 2001 lines
could be tens of megabytes while the transport refuses a command's stdout past
1 MiB with `StreamOverflowError`. A chat-heavy bot reaches that at about 524
bytes a line. The second bound is therefore `--no-full` under an explicit
`COLUMNS`, which is journalctl's own ellipsizing and needs no pipe, so the exit
status is still the reader's. Verified against systemd 252: each entry is cut to
`COLUMNS` columns, the timestamp-host-identifier prefix is never cut so every
line still parses, and the `-- cursor:` note is printed **outside** the
ellipsizing formatter and comes back whole even at `COLUMNS=40` — which is what
makes this bound safe, since a cut that took the cursor would leave the observer
re-reading one window for ever. No entry is dropped, so nothing is lost that the
`full` contract below does not already cover.

`JOURNAL_LINE_COLUMNS` is 130 because a column may cost 4 UTF-8 bytes — a
width-1 character outside the basic plane, which a bot that posts mathematical
alphanumerics really does log — so 2001 lines of 130 columns is 1 042 521 bytes,
under 1 MiB with room for the notes. What is cut is the tail of a long chat line;
what is kept is every marker, which sits at the front, and enough of a disconnect
reason for `looksLikeKick` to see "kick" or "banned". Raising either constant
past the other fails the test that reads both off the command.
...
| A first owner created from a browser only while this deployment has none, and nothing left behind when that fails | `apps/server/src/first-run-registration.test.ts` — drives `member.registerFirstOwner` over HTTP against a real `startServer` on its own database, in the order a deployment lives it: a **bare `user` row with no organization** already closes registration and is refused `REGISTRATION_CLOSED`, so a gate read off the `organization` table instead fails there rather than passing; registration then reads open while the `user` table is empty; a name carrying a NUL, a 129-character password and a whitespace-only organization name are each refused **400** with the table still empty and registration still open; the call then creates the owner, the organization and the membership whose role is `owner`; registration reads closed; a second call is refused `REGISTRATION_CLOSED` with one user still in the table; and that owner signs in. The route calls `bootstrapOwner`, so the refusal is the same `anyUserExists` check under the same advisory lock the CLI takes. `apps/server/src/bootstrap-owner.test.ts` holds the half HTTP can no longer reach now that the boundary refuses the input that provoked it: a `createOrganization` that fails must leave **no** `user`, `account`, `session` or `organization` row and the next bootstrap must succeed — deleting the compensation strands an account and closes an unclaimed deployment for ever, which is one anonymous request away from a lockout only direct database access can undo. The same file requires a deployment that already holds a user to be refused **while the bootstrap lock is held by someone else**, so moving the cheap pool-side check back under the lock hangs rather than passes, and refusals stop costing a connection each. `apps/server/src/oidc-sign-in.test.ts` requires registration to read open on an untouched deployment **in the same breath** as the provider's stranger being refused, and it is the only file that holds `bootstrap.ts`'s mounted `closed` mode — `registration-gate.test.ts` builds its own instances and is 12/12 green with the mounted one reverted to `gated`. `apps/web/src/routes/register.test.tsx` walks the page in both states and requires the sign-in page to offer the link in one and not the other. What it does NOT hold: two browsers racing the first registration, which is `bootstrap-owner.test.ts`'s own advisory-lock case; that `deleteUser` itself succeeding is guaranteed, since a compensation that throws leaves the account stranded and nothing drives that; nor anything about a **restored database** — emptying `user` reopens registration, which is reachable by a backup restore and is recorded in `SECURITY.md` rather than enforced |
...
| An exec's deadline bounding the caller's whole wait, including time queued behind the connection's channel limit | `packages/transport/src/ssh/connection.queue.test.ts` — saturates a real `ssh2` connection's six channel slots with commands that never exit, then requires a seventh exec given 250ms to end as `ChannelQueueExpiredError` inside a three-second watchdog, so returning the slot wait to outside the timer leaves it reading `still queued` and fails; a second case requires the host to have seen only the six commands, so satisfying the first by opening a seventh channel and abandoning it fails there; a third holds the six for a second and then requires a seventh given 1500ms to end within 2000ms, so bounding the queue and then handing the command a fresh full deadline — which makes the caller's worst case *longer* than before — fails rather than passing on the first two. What it does NOT hold: that any budget above is met — the sums those rest on are counted by `instance.controller.test.ts`, `authenticate.test.ts` and `host.controller.test.ts`, none of which can see elapsed time at all; nor that a queue expiry releases the claim, which `claimedExec` does not do |
...
| A command that could carry a credential refused before anything stores it, while the console still takes it | `apps/server/src/instance.test.ts` — drives **Set scheduled command** over HTTP against a real database: `/login hunter2` is refused 400 `INSTANCE_COMMAND_STORES_CREDENTIAL`, the whole wire answer compared, and the caller is then demoted to **`viewer`** and `listScheduledCommands` read back empty — that read is the surface the defect had, because `toScheduledCommandPublic` returns `row.command` verbatim and the procedure requires only `instance.read`. A second case requires `/cp backup world` and `/login hunter2` to get the **same whole answer**, so the one sentence has to stay true of a command that carries no password at all. A third sends the **same** string through `instance.sendCommand` and requires a **200** there beside the schedule's refusal, so moving the check into `sendableLine`, which both doors read, fails. A fourth drives a **stopped** bot and requires the console to answer `INSTANCE_NOT_RUNNING` where the schedule still answers the credential refusal, which is why the sentence names the running bot rather than offering the console flatly. `packages/core/src/instance/control.test.ts` pins the set `refuseStoredCredential` refuses — ten forms including a bare `login`, `//login`, `/l`, `/register`, `/unregister`, `/authme changepassword` and `/email setpassword` — against six it stores, among them `/login` with **no argument** and `/logistics open`, so a refusal that refuses everything fails rather than passing on the refusals alone; a second case names four it refuses that carry no password (`/cp`, `/log`, `/l`, `/reg` with any argument at all); a third requires the rule to judge the command **as it arrived** — `" /say hello"`, `"/say hello "`, `"!reco "` and `"  /home"` all stored — because comparing a **trimmed** copy against the original passed the whole suite while refusing all four, and nothing trims upstream of the schema; and a fourth requires `sendableLine("/login hunter2")` to still answer `//login hunter2`. `packages/core/src/instance/instance.controller.test.ts` requires the save to reject with **nothing upserted and nothing audited**, and requires a stored `/say Authorization: Bearer …` to be audited masked, so `redactCommand` at that site is not deleted as dead. `apps/web/src/lib/errors.test.ts` compares the sentence whole, requires it **not** to match `/reach|server/i`, and requires it to differ from `INSTANCE_COMMAND_DOUBLE_SLASH`'s, so the two refusals cannot be given one reason. What it does NOT hold: **a row an earlier build already stored, which still RUNS and not merely lists**. Nothing sweeps `instanceCommand`, whose `command` column is plain `text` under a not-empty and a single-line CHECK and nothing else; `runScheduledCommand` reaches `sendableLine`, which carries no credential check, and `instance.controller.test.ts`'s `audits a scheduled password command without the password, while the bot still receives it` asserts `//login hunter2\n` reaching the FIFO — so such a row keeps logging the bot in every day until someone deletes it. What that row does **not** leak is checked: both audits mask it, and `lastRunError` holds one of `scheduledRunFailure`'s six fixed sentences, pinned by the case that drives a host stderr carrying `token hunter2` and still requires "The command could not be sent" — its one message pass-through is `HostUnreachableError`, whose message is a connection reason and never command text. Nor the live door's policy beyond that one 200 — an operator may still type any credential at the console, deliberately. Nor any credential syntax outside `COMMAND_PATTERNS`: the refusal **is** `maskCommandCredentials(command) !== command`, so that list is the whole policy and nothing measures whether it is the right one |
...
| A failed-unit count the host refused to give reported as unknown, never as nothing failed | `packages/core/src/host/health.test.ts` — runs the rendered command under `/bin/sh` against a `systemctl` shell function that answers the way a missing user bus does, and requires a **non-zero** status, so the `2>/dev/null | grep -c . || printf 0` it replaced — which exits 0 printing `0` on a real shell, measured on systemd 252 — fails there while every fake-driven case stays green; two cases beside it require nothing listed to exit 0 counting 0 and two listed lines to count 2, so a command that simply always fails does not pass either. The same file drives `observeHost` over a scripted refusal and requires `failedUnits: null`, compared as a whole object, which also holds that the dead `reachable: true` is gone. `apps/web/src/components/host-health-badge.test.tsx` walks every `HostHealth` and requires the five labels distinct, and requires the refused one to read "Not readable" and to borrow no other state's word. What it does NOT hold: that `recordSeen` writes the null — its own repository test does not drive it; nor that any other host read is asked in a way the host can refuse, since `OS_RELEASE_COMMAND` still ends in a `printf` and always exits 0, which is harmless only because a null OS fact is already skipped on write |
| The journal read asking for fewer bytes than the transport will carry back | `packages/core/src/status/instance-observer.test.ts` — reads `COLUMNS=` and `-n` off the command `journalCommand` actually issues, for the seed read and the resumed one, and requires `lines × (columns × 4 + 1)` plus a note reserve below `MAX_STDOUT_BYTES`, treating a command that names no width or drops `--no-full` as unbounded, so the read as it stood fails there. Measured: `JOURNAL_LINE_COLUMNS` at 131 — one non-test file, every other case green — fails it alone, `expected 1054096 to be less than or equal to 1048576`. It proves the **arithmetic**, NOT that journalctl honours the width: `scripts/sandbox/journal-cursor.sandbox.ts` logs a line twice the width on Debian 12, Debian 13 and Ubuntu 24.04 and requires every such line back under the width, the full line never present, and the `-- cursor:` note still a cursor — the fact this bound rests on, since a cut that took the cursor would stall the observer on one window for ever. Nor does anything here drive a real overflow through a real transport: `StreamOverflowError` is still only ever constructed by hand |
| One bot whose journal could not be read not costing its host's other bots their round | `packages/core/src/status/instance-observer.test.ts` — drives `observeEachInstance` over three bots with a `StreamOverflowError` thrown for the middle one, and requires the third still read and the failure reported naming that bot, so the bare `for` loop this replaced — which `apps/server/src/bootstrap.ts` had inline, where the throw skipped every later bot on the host and, the cursor never saved, did so again on every poll — fails there. A third case requires a bot that could not be leased to stop the host's sweep, so widening the catch to swallow that too fails. What it does NOT hold: that `bootstrap.ts` reports the failure anywhere an operator will look — `onFailed` is wired to `runtimeErrorReporter`, a log line, which is where every other health-poll failure with no operator waiting already goes |
...
- **What may be sent is not what may be stored.** `sendableLine` answers whether a
  command can be sent at all and both doors read it; `refuseStoredCredential`
  (`instance/control.ts`) answers a second question only the **schedule** door
  asks, whether storing the command would leave a credential where other members
  can read it. `setScheduledCommand` calls both, `sendableLine` first, and that
  order is the copy: a command this manager would never send is refused for that
  reason at both doors, so the storage refusal never tells an operator to send at
  the console something the console would refuse too. Do **not** move the
  credential check into `sendableLine`. The console is where a password belongs —
  typed once, sent to the server, masked by `redactCommand` in the audit entry and
  kept out of `localStorage` by `command-history.ts` — and a schedule was the one
  surface of those four that still kept it whole: `toScheduledCommandPublic`
  returns `row.command` as stored and `listScheduledCommands` asks only for
  `instance.read`, which a **viewer** holds, while `audit.read` is an owner's
  alone. **Transmit yes, persist never** is already this project's rule, and this
  makes the third place that follows it: `command-history.ts` drops a command from
  `localStorage` on the identical predicate while the console still sends it, and
  `redactCommand` masks the audit entry of a command the bot did receive. The
  split between the doors is that rule, not a judgement call. Refusing at save
  rather than masking on read is deliberate: a masked schedule is one an operator
  cannot edit while the scheduler still runs the real command, so nothing
  sensitive being written is the only version of this that holds. It does **not**
  remove a row an earlier build stored, and such a row still **runs** rather than
  merely listing — `runScheduledCommand` reaches `sendableLine`, which carries no
  credential check. Nothing sweeps `instanceCommand`, and deleting an operator's
  schedule out from under them is a decision nobody has made. The policy is
  exactly `maskCommandCredentials(command) !== command`, so `COMMAND_PATTERNS` is
  all of it, and it is broader than "a password": any argument to `login`, `l`,
  `log`, `reg`, `register`, `unreg`, `unregister`, `cp`, `changepass`,
  `changepassword`, `authme register|reg|r|password|changepass|changepassword|cp`
  or `email setpassword`, behind up to two leading `/` or `!`, is now unsavable.
  Adding a pattern there narrows what can be scheduled as well as what is masked.
  **The refusal's sentence must therefore stay true of a command carrying no
  password at all** — those patterns were tuned for masking, where over-matching
  costs nothing — so it names the command's first word rather than claiming the
  command is a login, and it offers the console **while the bot runs**, because
  the schedule door has no running check and the console door does. It does not
  offer the console as a replacement for the schedule: the operator asked for a
  recurring command and there is none to give them.
- **A queue insert whose result is discarded is a data-integrity bug, not a copy
  ...
  argument, and it is not the argument. `delivery.job.ts`'s two checks — the
  requeue at the end of the retry branch and the set-aside at the end of the
  give-up branch — are the same rule inside the worker rather than behind a
  button, and both sit inside the same `withTransaction` as the `settleDelivery`
  they belong to, which is what the transaction is there for. Its factory is
  `createDeliveryTransaction`, in `delivery.job.ts` beside the handler and not
  inlined in `apps/worker/src/bootstrap.ts`, so the real transaction a test
  drives is the one the worker runs. **The rollback leaves the delivery
  reading `queued`, which is true but not free, and the cost is recorded here
  rather than smoothed over**: the row goes back to exactly what it read before
  the attempt, and nothing queues another attempt on its own. The delivery
  queues carry `retryLimit: 0`, so the pg-boss job the worker was running
  dead-letters; `listFailures` and `retry` take `failed` only, so the operator
  is never shown it and cannot ask for it again; and `deleteSettledBefore`
  deliberately keeps a notification that still has a `queued` delivery, so the
  retention sweep will not remove it either. That is a lost alert, and it is the
  same trade `announce.ts` already takes — lost rather than corrupt. It is still
  the right way round, because the alternatives commit a lie: a delivery
  `failed` with no `notification.deadletter` record behind it, or `queued` at a
  spent attempt with no job to spend the next one. Do not answer it with a
  sweeper over `queued` deliveries without deciding first what a delivery whose
  worker simply crashed should mean, which is the same row.
...
- **An exec's wait bounds what the caller waits, queueing included.** Every
  budget above sums the waits one claim covers and compares the total against
  its lease. That is arithmetic about elapsed time only if each wait actually
  ends. `createSshTransport` allows `DEFAULT_EXEC_CONCURRENCY` — six — session
  channels at once, and `exec` used to take its slot *before* arming the timer,
  so time spent queued behind six others sat outside the deadline the caller
  gave. Measured against a real OpenSSH `sshd`: eight concurrent execs, each
  given 4000ms, took **6053ms and none of them timed out**. No budget above was
  ever understated by it, because the queue is not reachable in production —
  every write-path transport is built by `createTransport()` for one operation
  and driven by one sequential flow, so at most one exec is ever in flight on
  it, and the shared read connections take `execUntil`, which the pool's own
  limiter bounds against the lease deadline instead. But those sums rested on
  that fact about call sites rather than on the transport, and the first
  `Promise.all` over two execs on one connection would have broken every one of
  them silently. The timer now covers the queue: the same eight execs end at
  4003ms, the two that queued at their own 4000ms deadline. A wait that runs
  out while still queued is `ChannelQueueExpiredError`, the name the read pool
  already gives it, and nothing reached the host. The residue: `claimedExec`
  keeps the claim for it, as it does for every rejection but a refused channel,
  although a queue expiry is one the manager can see the end of.
...
- `closed` (`alwaysRefuses`) — no `create-user` is ever admitted. **The instance
  ...
  by neither. That search is textual and cannot see a better-auth endpoint that
  creates a user without going through `signUpEmail` — the `admin` plugin's
  `createUser` is the example, and it is not installed — nor dispatch through a
  variable, nor a first-run dashboard flow written later. Every one of those
  fails **closed** on this instance: they would be refused `registration_closed`
  rather than silently admitted, which is the right way round, but a first-run UI
  added later must call `bootstrapOwner` and not reach for a mounted endpoint.
...
### Signing in through the operator's own provider

One generic OIDC provider, configured from the environment and absent entirely
when it is not. `OIDC_ISSUER_URL`, `OIDC_CLIENT_ID` and `OIDC_CLIENT_SECRET` set
...
configured. Two shapes of that one query parameter have already taken the page
down and both are pinned: the mapping is a `Map`, not an object, because an
object lookup resolves `Object.prototype` keys and `?error=toString` handed back
a **function**, which `useState` then called; and the search schema coerces,
because TanStack parses before zod hands over — `?error=42`, `?error=true`,
`?error=null` and a repeated `error` all arrive as a number, a boolean, `null`
and an array, and a bare `z.string()` rejected each of them into the error
boundary. Any test for this parameter needs a non-alphabetic case, or it covers
only one of the two doors. Configured, two routes join the five —
...
this costs no runtime dependency. The provider id is `OIDC_PROVIDER_ID` in
`packages/contracts`, read by the server for the callback path and by the
dashboard for the button, so the two cannot disagree about it.

`accountIssuer` is set explicitly to the configured issuer, and that is not
decoration. `genericOAuth`'s `init` fetches the discovery document, and
`betterAuth()` builds its context promise eagerly, once: an `init` that throws
makes every later `await` on that promise reject, so **password sign-in would
die with it**. With `accountIssuer` set, a provider that is down or serving
nothing at boot leaves the server up and only that button broken. Never remove
it to save a line.

`onAPIError.errorURL` is set to the dashboard's own sign-in page, and it is the
reason a reload is not a dead end. better-auth reaches for a fallback error
destination at four sites — `parseState`, and three in `callback.mjs` for a
callback carrying no `state`, an unparseable one, or a state record it cannot
find — and every one of them reads
`options.onAPIError?.errorURL || ${baseURL}/error`. `/api/auth/error` is not one
of the seven paths `auth-routes.ts` mounts and never will be, so before this was
set an operator who reloaded the callback URL, or pressed Back onto it, was
answered `302` to a **404** on the API origin. Measured against a real Keycloak:
the same callback twice gave `/hosts` and then
`http://localhost:28174/api/auth/error?error=state_mismatch`, and that address
answered `404 Not Found`. Mounting the route instead would not have fixed it:
better-auth's `/error` endpoint redirects to `/?<params>` **relative to the API
origin** whenever `NODE_ENV` is production and no `customizeDefaultErrorPage` is
given, and the dashboard is a different origin (`.env.example` ships `:25174`
against `:25173`), so the operator would land on a second 404; in development it
would serve better-auth's own branded page, which is not this product's. The
per-flow `errorCallbackURL` the dashboard sends still wins wherever the state
record survives — this only replaces the fallback for when it does not.

The address is `${ALLOWED_ORIGINS[0]}${SIGN_IN_PATH}`. `SIGN_IN_PATH` lives in
`packages/contracts` beside `OIDC_PROVIDER_ID`, read by the server for this
fallback and by the dashboard for its own `errorCallbackURL`, so the two cannot
disagree about where a failed sign-in lands. No new copy was needed and none was
added: `state_mismatch` is in no `SIGN_IN_FAILURES` entry, so the page says "That
sign-in did not finish. Try again, or sign in with your email and password." — a
sentence that blames nobody. The operator who reloaded usually never reads it,
because they already hold a session and `/sign-in`'s `beforeLoad` sends a
signed-in visitor straight on to `/hosts`.

The registration gate is what decides who arrives. An OIDC identity whose email
...
- The provider must assert `email_verified`, because `accountLinking.trustedProviders`
  is left empty and nothing may add this provider to it. Unconditional linking on
  an email string is an account-takeover vector whenever the provider will hand
  out an address its holder never proved; leaving the provider untrusted is the
  defence better-auth offers and we keep it. A member whose provider vouches for
  nothing is refused `account_not_linked` and reads a sentence saying so.
- `accountLinking.requireLocalEmailVerified` defaults to `true`, and **every** user
  here has `emailVerified: false`, because `signUpEmail` writes that and nothing
  in this product ever sends a verification mail. Left at its default, no invited
  operator could ever sign in this way. It is set to `false`, and only on an
  instance that carries a provider. What that default guards against — an attacker
  pre-registering an unverified account at a victim's address so the victim's
  OIDC identity links into the attacker's row — cannot happen here: nobody can
  register at all. The gate refuses every `create-user` once one user exists, and
  the bootstrap advisory lock makes exactly one first owner, so the only people
  who can cause a row at a given address to exist are the installer and a member
  holding `member.manage`. better-auth deprecates the option and says the gate
  will become unconditional; when that lands, `oidc-sign-in.test.ts`'s invited
  operator goes red rather than the feature failing quietly in a deployment.

Identity comes from the **ID token**, not from `/userinfo`, and that is what the
suite drives. Because a real discovery document advertises `jwks_uri` and
`issuer`, `genericOAuth`'s `init` builds an `idTokenConfig` around
`createRemoteJWKSet`, `getUserInfo` runs `verifyProviderIdToken` before it reads
anything, and `fetchUserInfo` takes `sub` and `email` straight off the decoded
token — so `/userinfo` is never called at all. Measured against a real Keycloak
with a logging proxy on the wire: discovery, the token exchange and a
`ua=jose/6.2.10` fetch of `/certs` appear, and no userinfo request appears
anywhere. The signature gates entry rather than decorating it: serving the wrong
realm's JWKS made the same user's sign-in fail
`unable_to_get_user_info`, and restoring it made the same flow succeed. The
`/userinfo` fallback is still reachable with a real provider — better-auth takes
it whenever the ID token carries no `email` — so the endpoint stays advertised
and served by the stand-in; nothing here says that branch is dead.

`requireIdTokenVerification` is deliberately **not** set on the provider config,
even though it reads like the hardening this section wants. `genericOAuth`'s
`init` throws when it is set and discovery gave no usable `jwks_uri`, and that
throw is the exact failure `accountIssuer` exists to prevent: it rejects the
eagerly built context promise and takes password sign-in down with it. A
provider that is down at boot must leave only its own button broken.

Whoever the operator points this at becomes the authority on their members'
email addresses. That is inherent to single sign-on and is said plainly in
`.env.example` rather than hidden.

The client secret is read from the environment into `createAuth` and goes to the
token endpoint in a `client_secret_post` body. It never reaches an authorization
URL, a cookie, a redirect, a response header or a log line, and no audit row is
written for a sign-in at all. `redact.ts` needs no pattern for it: a pattern
there masks a secret that has already reached a string somewhere, and this one is
never put into one — `redactValue` would be the tool if a call site ever had to
log something built from it.
