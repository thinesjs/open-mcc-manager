...
export const EXIT: MotionVariant = { opacity: 0, scale: 0.9, filter: "blur(2px)" }
...
export const decorativeDuration = (reduced: boolean, normal: number): number =>
	reduced ? Math.min(normal, 0.1) : normal

export const measuredDuration = (normal: number): number => normal
