...
- **Nothing under `packages/core/src` raises the bare `Error` class**, and
  ...
  of "Internal server error". Provisioning's own sites throw
  `HostProvisioningFailedError`, which moved to `host/provision.ts` so the file
