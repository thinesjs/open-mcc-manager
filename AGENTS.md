...
| A first owner created from a browser only while this deployment has none | `apps/server/src/first-run-registration.test.ts` — drives `member.registerFirstOwner` over HTTP against a real `startServer` on its own database: registration reads open while the `user` table is empty, the call then creates the owner, the organization and the membership whose role is `owner`, registration reads closed, a second call is refused `REGISTRATION_CLOSED` with one user still in the table, and that owner signs in. The route calls `bootstrapOwner`, so the refusal is the same `anyUserExists` check under the same advisory lock the CLI takes, and the browser's bit cannot widen it: making the bit always open leaves the refusal case failing. `apps/server/src/oidc-sign-in.test.ts` requires registration to read open on an untouched deployment **in the same breath** as the provider's stranger being refused, so opening registration cannot be mistaken for opening a door the provider can use. `apps/web/src/routes/register.test.tsx` walks the page in both states and requires the sign-in page to offer the link in one and not the other. What it does NOT hold: two browsers racing the first registration — `bootstrap-owner.test.ts` holds the lock; nor that the mounted instance stays `closed`, which is `registration-gate.test.ts`'s row above |
