...
| No sandbox container is given a host path, home directory, `~/.ssh` or the Docker socket | `scripts/sandbox/sandbox.test.ts` — every `docker` call the sandbox suite makes goes through one guard, which refuses any argument that is `-v` alone or at the end of a short-flag group, `--volume`, `--mount` or `--volumes-from`, any argument naming `docker.sock`, and every `docker cp` or `docker container cp` in either direction. Files reach a container on `docker exec` stdin. A direct `spawn("docker", ...)` would go around it |
