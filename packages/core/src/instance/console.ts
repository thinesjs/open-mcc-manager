...
export const readConsole = async (
	...
	if (!Number.isInteger(lines) || lines < 1 || lines > 1000) {
		throw new InternalError("Console line count must be a whole number between 1 and 1000")
	}
	...
	if (result.exitCode !== 0) {
		throw new HostRefusedError(`Failed to read console for instance ${id}: ${result.stderr.trim()}`)
	}
